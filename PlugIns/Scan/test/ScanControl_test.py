...
class TestScanControlClass(unittest.TestCase):
    ...
    def test_record_followed_by_view_updates_display(self):
        document_controller, document_model, hardware_source, scan_state_controller = self._setup_scan_hardware_source("a")
        with contextlib.closing(document_controller), contextlib.closing(scan_state_controller):
    ...
    def test_start_playing_with_existing_probe_position_graphic_should_remove_it(self):
        ...
        hardware_source = self._setup_hardware_source()
        document_model.setup_channel(hardware_source.hardware_source_id, hardware_source.data_channels[0].channel_id, data_item)
        HardwareSource.HardwareSourceManager().register_hardware_source(hardware_source)
        display = document_model.data_items[0].primary_display_specifier.display
