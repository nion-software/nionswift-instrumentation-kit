...
class AcquisitionController(Declarative.Handler):
    ...
    async def _acquire(self) -> _AcquireResult:
        ...
        def handle_acquire_error(error: Exception) -> None:
            nonlocal is_error
            is_error = True

        data_channel_provider = DataChannelProvider(self.document_controller)

        self.__acquisition = self.start_acquisition(device_component, method_component, data_channel_provider, handle_acquire_finished, handle_acquire_error)

        # wait for the acquisition to finish
        while self.__acquisition and not self.__acquisition.is_finished:
            await asyncio.sleep(0.1)

        acquire_result = _AcquireResult(not is_error, data_channel_provider._data_item_map)
...
class _AcquireResult:
    ...
    def summarize(self) -> None:
        print(f"Acquisition result: {self.success} {self.data_item_map}")
