...
class AcquisitionTestDialog(Declarative.Handler):
    def __init__(self, window: DocumentController.DocumentController) -> None:
        ...
        ui_view = u.create_column(
            u.create_scroll_area(u.create_column(items="acquisition_tests", item_component_id="acquisition_test", margin_vertical=4), min_width=480, min_height=400),
            u.create_row(
                u.create_stretch(),
                u.create_push_button(text="Reset", enabled="@binding(is_stopped_model.value)", on_clicked="handle_reset_clicked", style="minimal"),
                u.create_push_button(text="@binding(run_all_button_text_model.value)", enabled="@binding(run_all_enabled_model.value)", on_clicked="handle_run_all_clicked", style="minimal"),
                spacing=8, margin=8),
        )
